...
from protein_generator.datasets.utils import aa2atoms, aa2graph, protein_atomname_to_atomobj, peptide_btypes
...
class PDBChainData:
	...
	def __init__(self, parser, pdb_file, chain_id=None):
		self.parser = parser
	...
	def build_graph(self):
		...
		if self.chain_id:
			...
			chain = [chain for chain in structure.get_chains()]
		...
		protein_atoms, restypes, protein_coords, atom_names = self.convert_protein_to_atoms(residues)
		...
		src, dst, coords = torch.tensor(protein_src).to(dtype=torch.int64), torch.tensor(protein_dst).to(dtype=torch.int64), torch.tensor(protein_coords)
		G = dgl.graph((src, dst))
		G.edata["rel_pos"] = (
            (coords[dst, :] - coords[src, :])
            .detach()
            .to(dtype=torch.float32)
        )
		atom_features = torch.tensor(atom_features)
		node_features = {"0": atom_features, "1": torch.tensor(coords).to(dtype=torch.float32)}
		bond_features = torch.tensor(bond_features)
		edge_features = {"0": bond_features}
	...
	def convert_protein_to_atoms(residues):
		...
		return atoms, restypes, coords, all_atom_names
...
class RandomNoiseData:
	...
	def build_graph(self):
		...
		src, dst, coords = torch.tensor(src).to(dtype=torch.int64), torch.tensor(dst).to(dtype=torch.int64), torch.tensor(coords)
		G = dgl.graph((src, dst))
		G.edata["rel_pos"] = (
            (coords[dst, :] - coords[src, :])
            .detach()
	...
	def build_protein_backbone_bonds(self):
		...
		src_all, dst_all, bonds = [], [], []
		for curr_res_id in range(self.num_residues):
			restype = "GLY"
			res_graph = aa2graph[restype]
			atoms = aa2atoms[restype]
